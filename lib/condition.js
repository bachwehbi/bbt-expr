...
BBTExpr.ResourceValue = function (opts) {
  ...
  this.iscomplex = opts.iscomplex
  this.keys = opts.keys

  var self = this

  this.getTopicFromParams = function (cb) {
    async.parallel([
      function (callback) {
        self.channel.evaluate(callback)
      },
      function (callback) {
        self.resource.evaluate(callback)
      }
    ], function (err, results) {
      if (err) return cb(err)
      var retval = {
...
BBTExpr.ResourceValue.prototype.setTrigger = function (trigger) {
  BBTExpr.GenericCondition.prototype.setTrigger.call(this, trigger)

  if (this.iscomplex) {
    this.channel.setTrigger(this.trigger)
    this.resource.setTrigger(this.trigger)
  }

  return this
}

BBTExpr.ResourceValue.prototype.setConnector = function (connector) {
  BBTExpr.GenericCondition.prototype.setConnector.call(this, connector)

  if (this.iscomplex) {
    this.channel.setConnector(this.connector)
    this.resource.setConnector(this.connector)
  }
...
BBTExpr.ResourceValue.prototype.evaluate = function (callback) {
  ...
  self.getTopic(function (err, topic) {
    if (err) return callback(err)

    client(topic, function (err, reply) {
      if (err) {
        return callback (err)
      } else if (reply) {
        var val = reply
