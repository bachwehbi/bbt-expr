...
BBTExpr.AndCondition.prototype.evaluate = function (cb ) {
  var self = this;
  async.parallel([
    function ( callback ) { self.left.evaluate(callback ); },
    function ( callback ) { self.right.evaluate(callback ); }
...
BBTExpr.ResourceValue = function ( opts ) {
  ...
  this.getTopic = function () {
    return {channel: self.channel, resource: self.resource}
  }
...
BBTExpr.createExpressionOrValue = function ( opts ) {
  ...
  if( left.ctype === types.RAW_VALUE && !right /* for operators with one child like NOT */) {
    opts.val = evalExpression( left.getVal(), null, oper );
    opts.oper = types.OPERATORS.BBT_PASS; //just in case it was not set!!!
    left = null;
    var retval = new BBTExpr.Value( opts );
    return retval;
  } else if( left.ctype === types.RAW_VALUE && right.ctype === types.RAW_VALUE /* for operators with two children */) {
